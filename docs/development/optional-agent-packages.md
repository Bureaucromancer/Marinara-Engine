...
Packages holding the `prompt-context` permission contribute text to the system prompt of each generated turn, so a package that owns live state can keep the model consistent with what the player is looking at. A contribution may also declare which built-in game systems it replaces, and Engine then stops instructing the model to drive them. Contributions are collected per turn and are never required: a contributor that returns nothing is skipped, and one that throws is logged and skipped without affecting generation.
